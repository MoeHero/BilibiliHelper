class ModuleLogger {
    ...
    static printUntreated(json) {
        ...
        this.warning(`萌园助手 ${callerName} 未处理返回 ${JSON.stringify(json)}`);
    }

    static sign(msg) {
        this.info(`萌园助手 自动签到 ${msg}`);
    }
    static treasure(msg) {
        this.info(`萌园助手 自动领瓜子 ${msg}`);
    }
    static smallTV(msg) {
        this.info(`萌园助手 小电视自动抽奖 ${msg}`);
    }
    static activity(msg) {
        this.info(`萌园助手 活动自动抽奖 ${msg}`);
    }
}
